...
// new code for getting pools
import { sushiClient, blockClient } from '../../apollo/client'
import { SUSHI_POOLS } from '../../apollo/queries'

const sushiPools = getSushiPools(100).then(result => console.log(result));
...
async function getSushiPools(numPools) {
  let result = await sushiClient.query({
    query: SUSHI_POOLS,
    variables: {
      first: numPools
    },
  ...
  return result?.data?.masterChefPools
}

function PoolList({ pairs, color, disbaleLinks, maxItems = 10 }) {
      ...
      >
        ...
        {!below1080 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              Boost {sortedColumn === SORT_FIELD.VOL_7DAYS ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
        {!below1080 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              SUSHI/hr/$1k {sortedColumn === SORT_FIELD.FEES ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
        {!below1080 && (
